...
describe('[command] validate command', () => {
  describe('validateCommand', () => {
    ...
    test('returns error when command is null', () => {
      ...
      // Act
      const result = validateCommand(command as unknown as Command)

      // Assert
      expect(result.ok).toBe(false)
      if (!result.ok) {
        expect(result.error.code).toBe('INVALID_COMMAND')
        expect(result.error.message).toBe('Command is not valid')
      }
    })
    ...
    test('returns error when command is primitive string', () => {
      // Arrange
      const command = 'not-an-object'
    ...
    test('returns error when command is primitive number', () => {
      // Arrange
      const command = 42
    ...
    test('returns error when command is boolean', () => {
      // Arrange
      const command = true
    ...
    test('returns error when command is array', () => {
      // Arrange
      const command = [1, 2, 3]
